...
import { 
  Info, Sparkles, Globe, Leaf, UtensilsCrossed, Wine, Clock, ChefHat, Flame, Droplets, Wind, Zap, ArrowRight, Star, Film, BookOpen, Link as LinkIcon, ShieldAlert, RefreshCw
} from 'lucide-react';
import { AnalyzeDishResponse, NutritionEstimate, RecipeDetail } from './types'; // <-- Đã sửa đường dẫn import thành ./types

// --- SUB-COMPONENTS for AnalysisResult ---
...
const OverviewTab: React.FC<{ result: AnalyzeDishResponse }> = ({ result }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {result.cultural_significance && ( // Lưu ý: Cần đảm bảo trường này có trong types/response
          <div className="bg-cinematic-gray-light/30 rounded-2xl p-6 border border-gray-800">
...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ result, loading, onReset, errorType }) => {
  ...
  if (result) {
    return (
      <div className="bg-gradient-to-br from-cinematic-gray to-black rounded-3xl border border-gray-800 overflow-hidden shadow-2xl animate-slide-up relative">
        <div className="h-1 bg-gradient-to-r from-cinematic-accent via-cinematic-gold to-cinematic-accent relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/50 to-transparent animate-shimmer"></div></div>
        <div className="p-6 md:p-8 border-b border-gray-800 bg-gradient-to-b from-gray-900/50 to-transparent relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 text-cinematic-gold mb-3"><Sparkles className="w-4 h-4 animate-pulse" /><span className="text-xs font-bold uppercase tracking-widest">AI Nhận Diện Hoàn Tất</span></div>
            <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
              <div>
                <h2 className="text-4xl md:text-6xl font-display font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 leading-tight mb-1 tracking-wide">{result.dish_name}</h2>
                <div className="flex items-center gap-3 text-gray-400 text-sm mt-3"><div className="flex items-center gap-1.5"><Globe className="w-4 h-4" /><span className="text-white font-medium">{result.origin}</span></div>{result.recipe && (<><span className="text-gray-700">•</span><div className="flex items-center gap-1.5"><Clock className="w-4 h-4" /><span>{result.recipe.prepTimeMinutes + result.recipe.cookTimeMinutes} phút</span></div></>)}</div>
              </div>
              {result.health_tags?.length > 0 && (<div className="flex flex-wrap gap-2 justify-end max-w-xs">{result.health_tags.map((tag) => (<span key={tag} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-green-900/30 text-green-400 text-xs font-bold border border-green-500/30 backdrop-blur-sm"><Leaf className="w-3.5 h-3.5" /> {tag}</span>))}</div>)}
            </div>
            <div className="bg-black/30 backdrop-blur-sm p-5 rounded-2xl border border-gray-800/50"><p className="text-gray-300 leading-relaxed italic text-sm md:text-base">"{result.description}"</p></div>
          </div>
        </div>
        <div className="border-b border-gray-800 bg-cinematic-gray/30 px-6 md:px-8">
          <div className="flex gap-1 overflow-x-auto">{TABS.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-6 py-4 font-bold text-sm transition-all relative whitespace-nowrap ${activeTab === tab.id ? 'text-white' : 'text-gray-500 hover:text-gray-300'}`}><tab.icon className="w-4 h-4" />{tab.label}{activeTab === tab.id && (<div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-cinematic-accent to-cinematic-gold"></div>)}</button>))}</div>
        </div>
        ...
        <div className="p-6 md:p-8 pt-0">
          <div className="flex flex-col sm:flex-row gap-3">
            <button className="flex-1 py-4 text-base shadow-lg bg-gradient-to-r from-cinematic-gold via-amber-500 to-cinematic-gold text-black border-none font-bold hover:shadow-glow-gold transition-all relative overflow-hidden group rounded-xl"><div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-200%] group-hover:translate-x-[200%] transition-transform duration-700"></div><span className="flex items-center justify-center gap-2 relative z-10"><ArrowRight className="w-5 h-5" />Lưu Công Thức</span></button>
            <button onClick={onReset} className="px-6 py-4 bg-cinematic-gray-light border border-gray-700 hover:border-gray-600 text-gray-300 hover:text-white rounded-xl font-bold transition-all hover:scale-105">Phân Tích Món Khác</button>
          </div>
          <p className="text-center text-xs text-gray-600 mt-4 flex items-center justify-center gap-2"><Info className="w-3 h-3" /><span>Kết quả do AI tạo ra. Vui lòng xác minh thông tin quan trọng.</span></p>
        </div>
      </div>
    );
  }
