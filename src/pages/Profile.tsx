  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const userIdToUse = data.id || user.userId;
        const recipesData = await recipeService.getRecipesByUserId(userIdToUse);
    ...
    fetchData();
  }, [user]); // ✅ Dependency: khi user thay đổi (sau refresh), sẽ tự load lại

  const displayStats = {
    recipes: myRecipes.length,
    followers: profileData?.followerCount || 0,
    following: profileData?.followingCount || 0,
    likes: myRecipes.reduce((sum, recipe) => sum + (recipe.ratingsCount || 0), 0)
  };

  // ✅ Ưu tiên: profileData (mới nhất) → user (từ AuthContext) → fallback
  const displayImage = profileData?.profileImageUrl || user?.profileImageUrl || `https://ui-avatars.com/api/?name=${user?.displayName || user?.username}&background=random`;
  const displayName = profileData?.displayName || user?.displayName || user?.username;
      ...
      <div className="relative group mb-12 lg:mb-20">
        <div className="h-[300px] lg:h-[400px] w-full relative overflow-hidden">
          ...
          <img 
            ...
            alt="Profile Cover" 
            className="w-full h-full object-cover opacity-80 filter saturate-[0.8] contrast-[1.1]"
          />
                ...
                <div className="relative -mt-20 lg:-mt-0 lg:-ml-4 flex-shrink-0 group/avatar">
                  ...
                  <div className="w-32 h-32 lg:w-40 lg:h-40 rounded-full p-1 bg-gradient-to-b from-[#D4AF37] to-[#8a6e1e] shadow-[0_10px_40px_-10px_rgba(212,175,55,0.5)] relative z-10">
                    <div className="w-full h-full rounded-full border-4 border-[#121212] overflow-hidden bg-black relative">
                      {/* ✅ Hiển thị ảnh với cache busting */}
                      <img 
                        src={`${displayImage}${displayImage.startsWith('data:') ? '' : `?t=${Date.now()}`}`}
                        alt="Avatar"
                        className="w-full h-full object-cover transition-transform duration-500 group-hover/avatar:scale-110"
                        key={displayImage} // Force re-render khi URL thay đổi
                      />
                  ...
                  <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
                    <div>
                      ...
                      <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3 text-sm text-gray-400 mb-4">
                        <span className="text-[#D4AF37] font-medium">@{displayUsername}</span>
                    ...
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <button onClick={logout} className="p-2.5 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-500 transition-all" title="Đăng xuất">
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 mt-24 lg:mt-32">
        <div className="flex justify-center border-b border-white/10 mb-10">
          <div className="flex gap-8 relative">
            <button onClick={() => setActiveTab('recipes')} className={`pb-4 px-2 text-sm font-bold uppercase tracking-widest transition-all ${activeTab === 'recipes' ? 'text-[#D4AF37] drop-shadow-[0_0_10px_rgba(212,175,55,0.4)]' : 'text-gray-500 hover:text-gray-300'}`}>
              <span className="flex items-center gap-2"><Grid className="w-4 h-4" /> Bộ Sưu Tập</span>
              {activeTab === 'recipes' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-transparent via-[#D4AF37] to-transparent shadow-[0_-2px_10px_rgba(212,175,55,0.5)]" />}
            </button>
            <button onClick={() => setActiveTab('saved')} className={`pb-4 px-2 text-sm font-bold uppercase tracking-widest transition-all ${activeTab === 'saved' ? 'text-[#D4AF37] drop-shadow-[0_0_10px_rgba(212,175,55,0.4)]' : 'text-gray-500 hover:text-gray-300'}`}>
              <span className="flex items-center gap-2"><Heart className="w-4 h-4" /> Đã Lưu</span>
              {activeTab === 'saved' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-transparent via-[#D4AF37] to-transparent shadow-[0_-2px_10px_rgba(212,175,55,0.5)]" />}
            </button>
          </div>
        </div>
        ...
        <div className="min-h-[400px] animate-fade-in">
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1,2,3].map(i => <div key={i} className="h-80 rounded-2xl bg-white/5 animate-pulse border border-white/5"></div>)}
            </div>
          ) : activeTab === 'recipes' ? (
            <>
              <Link to="/recipe/submit" className="relative group block mb-12 overflow-hidden rounded-2xl border border-white/10 hover:border-[#D4AF37]/50 transition-colors">
                ...
                <div className="flex items-center justify-between p-6 lg:p-10 bg-[#121212] relative z-10">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-white mb-1 group-hover:text-[#D4AF37] transition-colors font-display">Sáng Tạo Món Mới</h3>
